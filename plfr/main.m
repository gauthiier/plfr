...
int main(int argc, const char * argv[]) {
    ...
    @autoreleasepool {
        ...
        for(NSUInteger i = 0; i < [doc pageCount]; i++) {
            ...
            NSArray *list = [p annotations];
            
            for(NSUInteger j = 0; j < [list count]; j++) {
                
                PDFAnnotation *a = [list objectAtIndex:j];
                
                if([[a type] isEqualToString:@"Highlight"]) {
                    
                    NSRect b = [a bounds];
                    PDFSelection *seltxt = [p selectionForRect:b];
                    ...
                    NSPoint p2 = [[quad objectAtIndex:2] pointValue];
                    
                    NSRect rb = NSMakeRect(b.origin.x, b.origin.y + p2.y, p0.x, p0.y - p2.y);
                    ...
                    // strip ending
                    NSUInteger l = [quad count];
                    NSPoint p3 = [[quad objectAtIndex:(l-1)] pointValue];
                    p2 = [[quad objectAtIndex:(l-3)] pointValue];
                    
                    NSRect re = NSMakeRect(b.origin.x + p3.x, b.origin.y, b.size.width - p3.x, p2.y);
                ...
                } else if([[a type] isEqualToString:@"Text"]){
                    
                    NSString *txt = [a contents];
